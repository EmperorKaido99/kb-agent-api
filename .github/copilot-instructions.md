# kb-agent-api - GitHub Copilot Instructions

## Prerequisites — Always Load Project Context

Before **any** planning, discovery, or implementation work, read `project-context.md` (repo root) in full. That file contains the authoritative rules for tech stack, coding conventions, testing, and critical anti-patterns. Follow those rules exactly — they take precedence over generic best practices.
...
When the user says **"StartTask TASK-XXXX"** (or similar), initiate the D5 workflow:
...
3. Create a task tracking file at `memorybank/activeTasks/TASK-XXXX.md` using the template below.
...
2. For each incomplete task, read its task file at `memorybank/activeTasks/TASK-XXXX.md`.
...
- Capture the ticket details (TASK-XXXX).
...
- **First:** Read `project-context.md` if not already loaded this session.
...
- **Update `TASK-XXXX.md` incrementally** after each discovery step — do NOT wait until the end.
...
- **Alignment check** — after updating docs, verify consistency across the three knowledge layers:
  1. `project-context.md` — tech stack rules, coding conventions, anti-patterns
  ...
  If any of these contradict each other after the task's changes, reconcile them now. For example: if a component doc references a validation pattern that `project-context.md` now forbids, update the component doc. If `features.md` lists behavior that a component doc shows has changed, update `features.md`.
...
When creating `memorybank/activeTasks/TASK-XXXX.md`, use this structure:
